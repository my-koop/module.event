...
var Module = (function (_super) {
    ...
    Module.prototype.getEvents = function (data, callback) {
        var events = [];
        this.db.getConnection(function (err, connection, cleanup) {
            ...
            var query = connection.query("SELECT e.idEvent, e.type, e.startDate, e.endDate, e.startAmount, e.endAmount, name, count(eu.idEvent) as countRegistered " + "FROM event e LEFT JOIN event_user eu ON eu.idEvent = e.idEvent WHERE e.endDate IS " + isNull + " NULL GROUP BY e.idEvent " + "ORDER BY e.startDate, e.endDate", [], function (err, rows) {
                cleanup();
                if (err) {
                    return callback(new DatabaseError(err));
                }
    ...
    Module.prototype.endEvent = function (id, endAmount, callback) {
        var queryData = {
            endAmount: endAmount,
        };
        this.db.getConnection(function (err, connection, cleanup) {
            ...
            var query = connection.query("UPDATE event SET endDate=NOW(), ? WHERE idEvent = ?", [queryData, id], function (err) {
                cleanup();
                if (err) {
                    return callback(new DatabaseError(err));
                }
    ...
    Module.prototype.startEvent = function (id, startAmount, callback) {
        var queryData = {
        ...
        this.db.getConnection(function (err, connection, cleanup) {
            ...
            var query = connection.query("UPDATE event SET startDate=NOW(), ? WHERE idEvent = ?", [queryData, id], function (err) {
                cleanup();
                if (err) {
                    return callback(new DatabaseError(err));
                }
    ...
    Module.prototype.registerToEvent = function (data, callback) {
        var queryData = {
            idUser: data.idUser,
    ...
    Module.prototype.getEvent = function (data, callback) {
        var event = null;
        this.db.getConnection(function (err, connection, cleanup) {
            if (err) {
                return callback(new DatabaseError(err));
            }
            var query = connection.query("SELECT * FROM event WHERE idEvent=?", [data.id], function (err, rows) {
                cleanup();
                if (err) {
                    return callback(new DatabaseError(err));
                }
